"""
Módulo de funciones de la práctica 5, regresión logística, de la asignatura de Machine Learning

Alejandro Useros Hernández
"""
...
class LogisticRegressor:
    ...
    def fit(
        ...
        #Dado que son las dimensiones de la tabla X, las obtenemos a la vez
        m,n = X.shape

        # TODO: Initialize all parameters to 0
        self.weights = np.zeros(n)
        self.bias = 0.0 #Obligamos a que sea un float, para no tener problemas más tarde
        ...
        for i in range(num_iterations):  #Lo rellenamos con el número de iteraciones
            ...
            #Calculamos el error
            error = y-y_hat
            dw = (-1/m) * np.dot(X.T, error)  # derivada w.r.t. weights
    ...
    def predict_proba(self, X):
        ...
        #El valor viene de multiplicar matricialmente los datos de X con self.w y sumarle el bias
    ...
    def predict(self, X, threshold=0.5):
        ...
        #Primero obtenemos las probabilidades en función de los datos
        probabilities = self.predict_proba(X)

        #El resultado de la clasificación será verdadero o falso en función de si estamos por encima del threshold
        classification_result = (probabilities>= threshold).astype(int)  #Devolvemos el valor en binario 1 True, 0 False
    ...
    def elasticnet_regularization(self, dw, m, C, l1_ratio):
        ...
        #Definimos ambos por separado para sumar luego
        l1 = l1_ratio * (C/m) * np.sign(self.weights)
        l2 = (1-l1_ratio) * (C/m) * self.weights
        #Sumamos ambos
        elasticnet_gradient = l1 + l2
        #Los devolvemos añadiendo el dw
    ...
    def log_likelihood(y, y_hat):
        ...
        #Simplemente aplicamos la fórmula que se nos da
    ...
    def sigmoid(z):
        ...
        - The sigmoid of z.
        """
        #Vemos que se pasa z como parámetro, que ya es w^T*X, luego simplemente hacemos el return de la fórmula

        # TODO: Implement the sigmoid function to convert the logits into probabilities

        #Lo asociamos al valor de la fórmula, usando numpy para la función exponencial
